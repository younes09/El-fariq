					...
					<div class="col-lg-12 col-md-12">
						<div class="card">
							<div class="card-body">
								...
								<form action="{{ url('team/'.$team->id) }}" method="POST" enctype="multipart/form-data" data-parsley-validate="">
									@csrf
									@method('PUT')
									<div class="row row-sm">
										<div class="col-lg-4 col-sm-12 mb-3">
											<label for="" class="form-label form-font">ماهو اسم الفريق:<span class="tx-danger">*</span></label>
											<input type="text" name="team_name" value="{{ $team->team_name }}" class="form-control" id="" required>                                
										</div>
										<div class="col-lg-4 col-sm-12 mb-3">											
											...
											<select name="team_prencipal_color[]" class="form-control select2" multiple="multiple" required>
												@php
													//show only selected input from database
													$p_color = explode("/", $team->team_prencipal_color);
													array_shift($p_color);
													$table = array("الأحمر","البرتقالي","الأصفر","الأخضر","الأزرق","البنفسجي","الأبيض","الأسود");
													$diff = array_diff($table,$p_color);

													foreach ($p_color as $p_col) {
														echo "<option selected value=\"$p_col\">$p_col</option>";
													}
													foreach ($diff as $t) {
										...
										<div class="col-lg-4 col-sm-12 mb-3">
											...
											<select name="team_second_color[]" class="form-control select2" multiple="multiple" required>
												@php
													//show only selected input from database
													$p_color = explode("/", $team->team_second_color);
													array_shift($p_color);
													$table = array("الأحمر","البرتقالي","الأصفر","الأخضر","الأزرق","البنفسجي","الأبيض","الأسود");
													$diff = array_diff($table,$p_color);
													$diffsec = array_diff($table,$p_color);
													foreach ($p_color as $p_col) {
														echo "<option selected value=\"$p_col\">$p_col</option>";
													}
													foreach ($diff as $t) {
												...
												<div class="card-body">
													...
													<div class="row mb-4">
														<div class="col-sm-12 col-md-4 mg-t-10 mg-sm-t-0">
															<input type="file" name="team_logo" class="dropify" data-default-file="{{url('images/'.$team->team_logo)}}" data-height="200" accept="image/*"/>
														</div>
													</div>
												</div>
											</div>
										</div>										
										<div class="col-12 text-center"><button class="btn btn-success pd-x-20 mg-t-10" style="font-size: large;" type="submit">تحديث المعلومات <i class="fas fa-check mr-3 "></i></button></div>
									</div>
								</form>
							</div>
						</div>
